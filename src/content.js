export const content = {
  nav:{
    logo:{
      src:"https://images.unsplash.com/photo-1524860769472-246b6afea403?ixid=MXwxMjA3fDB8MHxzZWFyY2h8OHx8aGFja2VyfGVufDB8fDB8&ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60",
    },
    links:[
      { text: 'Work', to:'work' },
      { text: 'Contact', to:'contact' }
    ]
  },
  header:{
    bgImg:{
      src:"https://images.unsplash.com/photo-1525373698358-041e3a460346?ixid=MXwxMjA3fDB8MHxzZWFyY2h8M3x8aGFja2VyfGVufDB8MXwwfA%3D%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=600&q=60"
    },
    texts:[
      "Hey !", "It's me Ashish Kumar - Fullstack Developer"
    ],
    typicalSteps:['MERN', 1000, 'MEAN', 1000, 'GRAPHQL', 1000, 'JAVA', 1000, 'NESTJS', 1000, 'FIREBASE', 1000, 'React Native', 100],
    about: 'Experienced Software Engineer skilled in JavaScript, Angular, Typescript, Node. js and React. js. Strong engineering professional with a Bachelor of Technology - BTech focused in Computer Science from Maharishi Markandeshwar University. I am a quick learner and always ready to learn something new.'
  },
  stack:[
    { src: "data:image/png;base64,", alt:"react" },
    { src: "data:image/png;base64,", alt:"angular" },
    { src: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSI457CcPOtN5vD4luo9sqivFSlgmPmYv7Jpw&usqp=CAU", alt:"node" },
    { src: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQeos16zOyUrE6r71N8HEjfzAflYCmFS3vMZQ&usqp=CAU", alt:"express" },
    { src: "data:image/png;base64,", alt:"mongodb" },
    { src: 'data:image/png;base64,', alt: "graphql" }
  ],
  projects:[
{
 name: 'Paint App',
 techUsed:['JS', 'Html', 'Css'],

 keyPoints:[
        'A canvas app developed to draw and download any quick drawing.',
        'Supports both mobile and web.'
      ],
      date: '08/2022 - 03/2024',
      link: 'https://ashish19977-paint.netlify.app',
},
    {
      name: 'Job Listing',
      techUsed:['JS', 'Express', 'Redis', 'React', 'Html', 'Css'],
      keyPoints:[
        'A scheduled job is kept running in backend for fetching all jobs from github.',
        'All fetched jobs are kept in redis in-memory db for faster access.',
        'Ant-d and react are used for making front-end(responsive)',
      ],
      date: '02/2021 - 03/2021',
      link:'https://joblisting-frontend.herokuapp.com/'
    },
    {
      name: 'Schedule Job Manager',
      techUsed:['JS', 'Express', 'MongoDB', 'Angular', 'Html', 'Css'],
      keyPoints:[
        'A flow was created to maintain the scheduled jobs running in backend.',
        'From front-end we can check health of jobs and their other stats.',
        'A option was provided to trigger a failed schedule job from front-end.',
      ],
      date: '12/2020 - 1/2021',
    },
    {
      name: 'News App',
      techUsed:['JS', 'React Native', 'Html', 'Css'],
      keyPoints:[
        'We are fetching news from a API and validating them.',
        'Showing news in Different languages and categories.',
        'Option to listen to news for individual news.',
      ],
      date: '11/2020 - 12/2020',
    },
    {
      name: 'Instant Chat App',
      techUsed:['JS', 'Express', 'Html', 'Css'],
      keyPoints:[
        'First we create a room and share that room name with anyone.',
        'Anyone having name of the room can join room and chat.',
        'Options to share images and live location in room.'
      ],
      link: 'https://instant-chats-with-black-theme.onrender.com',
      date: '09/2020 - 10/2020'
    },
    {
      name: 'Permissions-Demo App',
      techUsed:['JS', 'React Native', 'Html', 'Css'],
      keyPoints:[
        'A simple app with nice ui for basic permissions demo.',
        'Have functionalities like audio-video, location, camera, gallary.',
        'First app will ask for permission and then there will a demo.'
      ],
      link: '',
      date: '09/2020 - 10/2020'
    },
    {
      name: 'Calendar',
      techUsed:['React', 'JS', 'Html', 'Css'],
      keyPoints:[
        'React is used for front-end for this project',
        'We display the dates of current month along with the todos for selected date.',
        'Options to Add todo, edit todo, check past todos and set themes for app.'
      ],
      link: 'https://ashish1997-calendar.netlify.app',
      date: '04/2020 - 05/2020'
    },
  ],

  projectColors: {
    'Html':'#9e2b2b',
    'MongoDB': '#2b9e3c',
    'graphQL': '#9e2b8d',
    'Express': '#2b9e4c',
    'Css': '#2b959e',
    'Angular': '#9e2b65',
    'React': '#2b539e',
    'React Native': '#2b539e',
    'JS': '#7f2b9e',
    'Redis': '#3c3b9c',
    'Python': '#1a5276',
    'Django': '#0b5345',
  },
  work:[
    { date: 'Current - ', isCurrent: true, postion: 'Software Engineer', company: 'Siemens - Mendix', link:'https://www.mendix.com' }, 
    { date: '07/22 - 09/25', postion: 'Software Engineer', company: 'Velotio', link:'https://www.velotio.com' },    
    { date: '08/21 - 07/22', postion: 'Software Developer', company: 'Gaana.com', link:'https://gaana.com' },    
    { date: '10/20 - 7/21', isCurrent: false, postion: 'Software Associate', company: 'Phablecare', link:'https://www.phablecare.com/' },
    { date: '07/20 - 10/20', isCurrent: false, postion: 'Intern', company: 'Phablecare', link:'https://www.phablecare.com/' },
  ],
  socialLinks: [
    {
      name: 'github',
      imgUrl: 'https://img.icons8.com/bubbles/60/000000/github.png',
      url: 'https://github.com/ashish19977',
    },
    {
      name: 'linkedin',
      imgUrl: 'https://img.icons8.com/bubbles/60/000000/linkedin.png',
      url: 'https://linkedin.com/in/ashish-kumar-215264188'
    }
  ]
}
